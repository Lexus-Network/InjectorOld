...
/*
This is using version 3.1.4 of the wxWidgets Cross Platform Gui Libary!
https://wxwidgets.org/news/2020/07/wxwidgets-3.1.4-released/
This libary is not included in this solution and needs additional installation to compile!
*/
...
cMain::cMain() : wxFrame(nullptr, wxID_ANY, "Lunes Client Injector", wxDefaultPosition, wxSize(297.5, 162.5), wxMINIMIZE_BOX | wxSYSTEM_MENU | wxCAPTION | wxCLOSE_BOX | wxCLIP_CHILDREN) {
    ...
    CreateStatusBar(1);
    SetStatusText("Version 1.0 | Team Lunes", 0);
    ...
    taskBarControl.SetIcon(icon, "Double-Click to show Lunes Injector");

    notification = new wxNotificationMessage("Lunes Client Injector", "Lunes Client Injector is now hidden in your system tray", this, 0);
    notification->UseTaskBarIcon(&taskBarControl);
    openDialog = new wxFileDialog(this, "Select the .dll file", working_dir, "Lunes.dll" ,"Dynamic link library (*.dll)|*.dll", wxFD_OPEN);
}
...
void cMain::OnHideButton(wxCommandEvent& evt) {
    ...
    // Lunes Client Injector is now hidden in system tray
	evt.Skip();
}
