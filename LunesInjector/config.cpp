...
bool config::saveConfig() {
    ...
    if (create.is_open()) {
        ...
        wxMessageBox("Can't create config file!", "Lunes Client ERROR", wxICON_ERROR);
...
std::wstring config::makeConfig() {
    ...
    configstr += L"#Lunes Client injector config file\n";
...
void config::analyseState() {
    if (name == "customProcName") {
        ...
        wxMessageBox("\"" + name + "\" Is not a known Entry\nDeleting the config file might help!", "Lunes Config WARNING", wxICON_INFORMATION);
    }
}